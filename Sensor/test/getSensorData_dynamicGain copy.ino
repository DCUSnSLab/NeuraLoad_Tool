...
void configureSensor() {
  ...
  tsl.setTiming(TSL2591_INTEGRATIONTIME_300MS); // 100ms
}
...
void loop() {
    ...
    static int gainAdjustStep = 0;
    
    // 조도 센서 데이터 수집 시작 (10초마다)
    if (currentTime - lastLightSensorTime >= 10000 && !isCollectingLightData) {
        isCollectingLightData = true;
        lightDataStartTime = currentTime;
        gainAdjustStep = 0;
    }
    
    // 조도 센서 데이터 수집 중 (단계별 처리)
    if (isCollectingLightData) {
        unsigned long elapsed = currentTime - lightDataStartTime;
        
        if (elapsed >= 2000) { // 2초 후 완료 (게인 조정 시간 포함)
            // 게인 자동 조정이 완료된 후 센서 데이터 수집
        ...
        } else if (elapsed >= 50 && gainAdjustStep == 0) {
            // 50ms 후 게인 자동 조정 시작
            autoAdjustGain();
            gainAdjustStep = 1;
        }
...
void autoAdjustGain() {
    ...
    tsl2591Gain_t currentGain = tsl.getGain();
    ...
    for (int i = 0; i < 4; i++) {
        tsl.setGain(gains[i]);
        delay(400);  // 게인 변경 후 안정화 시간 (integration time보다 길게)
        ...
        if (ch0 >= 65535 || ch0 < 100) {
            score = 0;  // 포화 상태이거나 너무 낮음
        } else if (ch0 >= 10000 && ch0 <= 50000) {
            score = 100;  // 최적 범위
        } else if (ch0 >= 1000 && ch0 <= 60000) {
            score = 80;   // 좋은 범위
        } else if (ch0 >= 500) {
            score = 40;   // 사용 가능한 범위
        } else {
    ...
    // 현재 게인이 충분히 좋다면 변경하지 않음
    if (currentScore >= 80 && bestScore - currentScore < 20) {
        tsl.setGain(currentGain);  // 원래 게인으로 복원
        return;
    }
