...
class COGPositionMassEstimation_v6(AlgorithmBase):
    def __init__(self, name: str):
        ...
        self.coefficient = np.array([35.74, 32.22, 35.06, 30, 30.67, 31.49, 32.66, 32.01, 32.70])
    ...
    def preprocess_data(self, frame: SensorFrame, init_value: List[float]) -> Dict[str, Any]:
        ...
        filtered_deltas = np.array(deltas) * self.sensorWeights
    ...
    def estimate_weight(self, zCenter: float, i1: int, i2: int, ratio1: float, ratio2: float):
        ...
        if zCenter > 0:
            ...
            # print(f"weight1: {loc1_weight:.6f}, weight2: {loc2_weight:.6f}, coeff1: {coeff1:.6f}, coeff2: {coeff2:.6f}, i1: {i1}, i2: {i2}, coefficient[i1]: {self.coefficient[i1]:.6f}, coefficient[i2]: {self.coefficient[i2]:.6f}, zCenter: {zCenter:.6}")
            print(f"py deltas: {self.deltas[0][0]:.4f}, {self.deltas[1][0]:.4f}, {self.deltas[2][0]:.4f}, {self.deltas[3][0]:.4f} | weight1: {loc1_weight:.6f}, weight2: {loc2_weight:.6f}, coeff1: {coeff1:.6f}, coeff2: {coeff2:.6f}, i1: {i1}, i2: {i2}, coefficient[i1]: {self.coefficient[i1]:.6f}, coefficient[i2]: {self.coefficient[i2]:.6f}, zCenter: {zCenter:.6f}")
